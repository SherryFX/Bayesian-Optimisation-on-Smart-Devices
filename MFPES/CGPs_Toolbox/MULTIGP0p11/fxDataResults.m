...
%	
%
%	[RMSE, NMSE] = FXDATARESULTS(DATASETNAME, KERNTYPE, APPROX,
%	EXPERIMENTNO) Shows the results of the given demo for the FX data
%	set.
%	 Returns:
%	  RMSE - Root mean square error for each output.
%	  NMSE - Normalised root mean square error for each output.
%	 Arguments:
%	  DATASETNAME - The data set to load.
%	  KERNTYPE - The kernel used in the simulation ('sim' or
%	   'simwhite').
%	  APPROX - The approximation used ('ftc' or 'dtc').
%	  EXPERIMENTNO - Experiment number used in the demo.
%
%	[RMSE, NMSE] = FXDATARESULTS(DATASETNAME, KERNTYPE, APPROX,
%	EXPERIMENTNO, BASEDIRRESULTS, FLAGS)
%	 Returns:
%	  RMSE - Root mean square error for each output.
%	  NMSE - Normalised root mean square error for each output.
%	 Arguments:
...
%	  APPROX - The approximation used ('ftc' or 'dtc').
%	  EXPERIMENTNO - Experiment number used in the demo.
%	  BASEDIRRESULTS - Base directory for the result's file.
%	  FLAGS - (1) whether the results should be plotted on the screen or
%	   not. (2) whether the figures should be saved. (3) whether a
%	   summary of the errors and likelihood of the model is shown at the
%	   end.
%	
%
%	See also
%	DEMSIMDTCFXDATA, DEMSIMWHITEDTCFXDATA, LOADFXDATA


%	Copyright (c) 2009 David Luengo
% 	fxDataResults.m SVN version 395
...
elseif strcmp(options.compOptions.approx, 'dtc')
    Xt = linspace(count(1), count(end), 500)';
    [mu, varsigma] = multigpPosteriorMeanVar(testModel.comp{1}, Xt);
    [predVal, varsigma2] = multigpPosteriorMeanVar(testModel.comp{1}, XTestPlot{1});
else
...
if exist('demPpcaFxDataResults.mat', 'file')
    load demPpcaFxDataResults;
end
...
%                 e = plot(count, yEst(:, k-testModel.comp{1}.nlf), 'r');
    ...
    if k > testModel.comp{1}.nlf 
        if strcmp(options.compOptions.approx, 'ftc')
            rmse(k-testModel.comp{1}.nlf) = sqrt(mean((predVal{k} - yTestPlot{1}{k}).^2));
        elseif strcmp(options.compOptions.approx, 'dtc')
            rmse(k-testModel.comp{1}.nlf) = sqrt(mean((predVal{k} - yTestPlot{1}{k-testModel.comp{1}.nlf}).^2));
        else
            % TBD
        end
    end
