...
% 	simwhiteXrbfwhiteKernGradient.m SVN version 285
% 	last update 2009-03-04T16:40:21.660137Z
...
g1(1) = sum(sum( ((-deltaT+decay/invWidth).*K  ...
    + variance*sensitivity/sqrt(2*pi*invWidth) ...
...
g2(1) = sum(sum( (-0.5*((decay/invWidth)^2).*K  ...
    + variance*sensitivity/sqrt(8*pi*invWidth) ...
        * exp(-decay*deltaT + 0.5*(decay^2)/invWidth) ...
...
g1(2) = sum(sum(K .* covGrad)) / variance;
