...
% 	rbfwhiteXrbfwhiteKernGradient.m SVN version 285
% 	last update 2009-03-04T16:40:20.330099Z
...
gradErf1 = sqrt(2/pi) * (gradNu1Erf1...
    ...
    - exp(-0.5*invWidth1*invWidth2*(invWidthStat.*abs(deltaT)/(invWidth1+invWidth2)).^2) ...
        .* gradNu2Erf1);
g1(1) = sum(sum( (-0.5 * (deltaT.^2) .* K * gradNu1 ...
    + variance/sqrt(8*pi) * gradErf1 ...
...
gradErf2 = sqrt(2/pi) * (gradNu1Erf2...
    ...
    - exp(-0.5*invWidth1*invWidth2*(invWidthStat.*abs(deltaT)/(invWidth1+invWidth2)).^2) ...
        .* gradNu2Erf2);
g2(1) = sum(sum( (-0.5 * (deltaT.^2) .* K * gradNu2 ...
    + variance/sqrt(8*pi) * gradErf2 ...
        .* exp(-0.5*invWidth1*invWidth2*(deltaT.^2)/(invWidth1+invWidth2))) ...
    .* covGrad));
