...
Name = 'Branin';
experimentNo = '_multi_PES_l';
% path = 'C:\Users\Yehong\Dropbox\Matlab\BO\MTPES\';
% path = '/home/yehong/Matlab/BO/results/';

load('branin_currin_t');

noise = [sqrt(0.001), 0.01];    % noise variance for each task
cost = [10, 1];
N = 500;    % maximal no. of observations
M = 2;  % no. of output types
nlf = 2;    % no. of latent functions
...
task = {@target_Branin, @Currin, Name, noise};
...
for loop = 1:T
    ...
    while S <= N-cost(t)
        ...
        [result.umax_f(loop, S+cost(type)), result.umax_x(loop, it+1, :)] = getMaxMean(model, xmin, xmax, task, t, opts);
